...
var defaultStrumX2:Array<Float> = [690,802,914,1026];
...
override function update(elapsed:Float){time += elapsed;
	...
	vhs.data.iTime.value = [-1*Math.sin(time)];
	{
		if (moveing)
		{
			for (i in 0...cpuStrums.members.length) cpuStrums.members[i].x = defaultStrumX[i]+ 32 * Math.sin((time + i*0.25) * Math.PI);
			for (i in 0...playerStrums.members.length) playerStrums.members[i].x = defaultStrumX2[i]+ 32 * Math.sin((time + i*0.25) * Math.PI);
		}
	}
	{
		if (!moveing)
...
if (FlxG.save.data.glitch) {glitch.data.on.value = [0.];}
	...
	case 912:
		moveing = true;
if (FlxG.save.data.glitch) {glitch.data.on.value = [0.1];}
		defaultCamZoom = 0.88;
